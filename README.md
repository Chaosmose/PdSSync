...
One hashMap per root folder, the hash map is crypted and is always generated by the master.
...
2. Master -> proceed to delta hash-map
3. Master -> upload files with a .download prefix to the sync (according to the delta)
4. Master -> uploads the hasMap of a given channel and finalize the transaction (un prefix the files, and call the sanitizing procedure =  removal of orphans)
Optionaly the synch server can send a push notification to client to force the step 5.

5. Slave -> downloads the hashMap
6. Slave -> proceed to delta hash-map
...
## Key notions ##
For PdSSync a **hashMap** or a **subHashMap** is a dictionary with for a given folder a list of all its files and folder relative path as a key and a CRC32 of the file as a value.
